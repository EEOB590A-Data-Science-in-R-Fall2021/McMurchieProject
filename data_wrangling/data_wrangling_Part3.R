...
library(readr)
transplant <- read_csv("~/EEOB590A_DataScienceR/CourseMaterials/data/tidy/transplant_tidy_clean.csv")
view(transplant)
...
(transplant_notanao <- transplant %>%
    filter(site != "anao")) #note the use of  == not =

# Note - adding the parentheses around the whole thing makes it produce a
# variable and print the head of the tibble in the console. 
...
(transplant_anaonbig <- transplant %>%
    filter(site == "anao" | webarea > 0.2))

(transplant_anaosmall <- transplant %>%
    filter(site == "anao" & webarea < 0.2))

# Another way to do this is to use the %>% function, which can be used to
#identify if an element belongs to a vector or dataframe. For example,
#this will select every row where the site is anao, ladt, or forb
...
# keep rows where startdate is before 2013-07-30 and end date is before 2013-08-02 and webpres is yes

(transplant_myturn2 <- transplant %>%
...
# can select all columns between two columns
(transplant_select3i <- transplant %>%
    select(4:6))
...
# Can use select() in conjunction with the everything() helper.
...
(transplant_summ3 <- transplant %>%
    filter(websize > 50) %>%
    summarise(avglarge = mean(websize)))
...
with(transplant, table(site)) #same thing as below
table(transplant$site) #same thing as above
...
transplant %>%
  group_by(island) %>%
  summarize (avg = mean(websize)) #mean websize for each island
...
# Your turn: Calculate the mean, median, min, and max duration
#for webs with and without netting on Guam and Saipan, 
#and calculate the number of webs in each group
...
# Your turn, advanced version: 1. Calculate mean duration of webs with and
#without netting at each site on each island. 
#2. Add these average duration values to the tidy transplant dataset (the one we loaded at the start of this script).
#3. Select just the island, site, netting, mean_dur, web, websize, duration columns.
#4. Add a column that calculates the difference between duration of each web and
...
(transplantmyturn <- transplant %>%
    group_by(island, site, netting) %>%
    summarize(mean_duration = mean(duration), .groups = "drop") %>%
    left_join(transplant, by = c("island", "site", "netting")) %>%
    select(island, site, netting, mean_duration, web, websize, duration) %>%
    mutate(difference_meanduration = duration - mean_duration))