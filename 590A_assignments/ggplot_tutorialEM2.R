...
pstraps <- read.csv("~/EEOB590A_DataScienceR/CourseMaterials/data/tidy/pstraps.csv")
...
ggplot(pstraps, aes(mindist, prop, color = island)) +
  geom_point() +
  scale_color_brewer(type = "div", palette = 8) #diverging colors. try "seq", and "qual" for sequential or qualitative
...
ggplot(pstraps, aes(mindist, prop, color = island)) +
  ...
  theme(axis.text.x = element_text(size = 9),
        ...
        legend.key = element_blank(), # takes out grid lines Haldre hates
...
ggsave("mindist.pdf", plot = mindist_graph, width = 4, height = 4, units = "in", dpi = 300)
